...
**现版本为0.3.0beta，文档暂时未更新，新建项目功能暂时不可用
## 安装
依赖
```
python >= 3.6
...
启动DLNest Shell前端（推荐）:
```bash
python -m DLNest.Client.Client [-u <url>]
```
默认url为127.0.0.1:9999  
或可使用DLNest命令行前端:
```bash
python -m DLNest.Client.Simple [-u <url>]
```
## 使用
### 1. 新建基于DLNest的项目
在DLNest中，键入
...
```
load -r <某次训练的保存文件夹的绝对路径，如/root/proj/Saves/2020-09-10_12:00:00_0> -c <想要载入的epoch编号> [-s] <运行实验的脚本文件目录,默认维save所在Saves上级目录下的AnalyzeScripts> [-m] <估计的显存占用(MB)，默认为显卡满显存的90%>
```
...
|change_time_delay  |post|修改同一张卡两次任务的时间间隔(s)
